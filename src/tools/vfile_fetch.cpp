...
int VFileFetch(const std::string &sFilePath, const int iOffset,
               const int iFetchCount) {
  ...
  while (true) {
    ...
    bool bBufferValid = oState.ParseFromArray(
        g_oTmpBuffer.GetPtr() + sizeof(uint64_t), iLen - sizeof(uint64_t));
    if (!bBufferValid) {
      printf("This instance's buffer wrong, can't parse to acceptState, "
